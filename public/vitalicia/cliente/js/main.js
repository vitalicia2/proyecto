;(function () {
	...
	var burgerMenu = function() {
		...
		$('body').on('click', '.js-insideV-nav-toggle', function(event){
	...
	var clickMenu = function() {
		...
		$('a:not([class="external"])').click(function(event){
		    ...
		    if ( navbar.is(':visible')) {
		    	...
		    	$('.js-insideV-nav-toggle').removeClass('active');
		    }
	...
	var windowScroll = function() {
		...
		$(window).scroll(function(event){
		   	...
		   	var header = $('#insideV-header'),
				scrlTop = $(this).scrollTop();

			if ( scrlTop > 500 && scrlTop <= 2000 ) {
				header.addClass('navbar-fixed-top insideV-animated slideInDown');
			} else if ( scrlTop <= 500) {
				if ( header.hasClass('navbar-fixed-top') ) {
					header.addClass('navbar-fixed-top insideV-animated slideOutUp');
					setTimeout(function(){
						header.removeClass('navbar-fixed-top insideV-animated slideInDown slideOutUp');
					}, 100 );
				}
			} 
	...
	var servicesAnimate = function() {
		...
		if ( $('#insideV-our-services').length > 0 ) {	
			$('#insideV-our-services .to-animate').each(function( k ) {
	...
	var servicesWayPoint = function() {
		...
		if ( $('#insideV-our-services').length > 0 ) {
			$('#insideV-our-services').waypoint( function( direction ) {
	...
	var featuresAnimate = function() {
		...
		if ( $('#insideV-features').length > 0 ) {	
			$('#insideV-features .to-animate').each(function( k ) {
	...
	var featuresWayPoint = function() {
		...
		if ( $('#insideV-features').length > 0 ) {
			$('#insideV-features').waypoint( function( direction ) {
	...
	var testimonialsAnimate = function() {
		...
		if ( $('#insideV-testimonials').length > 0 ) {	
			$('#insideV-testimonials .to-animate').each(function( k ) {
	...
	var testimonialsWayPoint = function() {
		...
		if ( $('#insideV-testimonials').length > 0 ) {
			$('#insideV-testimonials').waypoint( function( direction ) {
	...
	var pricingAnimate = function() {
		...
		if ( $('#insideV-pricing').length > 0 ) {	
			$('#insideV-pricing .to-animate').each(function( k ) {
	...
	var pricingWayPoint = function() {
		...
		if ( $('#insideV-pricing').length > 0 ) {
			$('#insideV-pricing').waypoint( function( direction ) {
	...
	var pressAnimate = function() {
		...
		if ( $('#insideV-press').length > 0 ) {	
			$('#insideV-press .to-animate').each(function( k ) {
	...
	var pressWayPoint = function() {
		...
		if ( $('#insideV-press').length > 0 ) {
			$('#insideV-press').waypoint( function( direction ) {
